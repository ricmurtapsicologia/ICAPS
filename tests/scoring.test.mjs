...
test('all responses 1 produce expected construct endpoints', () => {
  const result = scoreAssessment(definition, responses(1));
  for (const d of result.dimensions) assert.equal(d.score, 0);
});

test('all responses 3 produce 50 in all dimensions', () => {
  const result = scoreAssessment(definition, responses(3));
  for (const d of result.dimensions) assert.equal(d.score, 50);
});

test('all responses 5 produce 100 in all constructs', () => {
